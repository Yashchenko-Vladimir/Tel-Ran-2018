...
AbstractRentCompany implements Persistable{
	...
	public CarsReturnCode returnCar(String carNumber, LocalDate returnDate, int gasTankPercent,
			...
			if(!carRecords.containsKey(carNumber)){
				...
				tmp.setCost(calculate_cost(tmp,models.get(modelName)));
	...
	@Override
	public CarsReturnCode removeCar
	(String carNumber) {
		Car car = getCar(carNumber);
    ...
    public List<Car> clear(LocalDate currentDate, int days) {
        ...
        NavigableMap<LocalDate, List<RentRecord>> recordsBefore =
        		returnedRecords.headMap(removeDate, true);
	...
	public List<String> getMostPopularModelNames() {
	     Map<String, Long> collect = getAllRecords()
	                .map(x -> getCar(x.getCarNumber()))
	                .collect(Collectors
	                		.groupingBy(Car::getModelName,
	                		Collectors.counting()));
	        long size = collect.values().stream()
	                .max(Long::compare)
	                .orElse(0l);
	...
	public double getModelProfit(String modelName) {
		 if(!models.containsKey(modelName))
			 return -1;
	        return getProfitFromRecords(getAllRecords()
	        		.filter(r->getModelName(r)
				 .equals(modelName)),modelName);
	}
	...
	public List<String> getMostProfitModelNames() {
		Map<String,Double>mapProfits=
			...
			(this::getModelName,Collectors.summingDouble
			(r->getProfitFromRecord(r,getModelName(r)))));
		double max=mapProfits.values().stream()
				.max(Double::compare)
                .orElse(0.0);
		List <String> res=new ArrayList<>();
	...
	@Override
	public Stream<RentRecord>
	getReturnedRecordsByDates(LocalDate from, LocalDate to) {
		return returnedRecords.subMap(from, to).values()
				.stream()
				.flatMap(List::stream);
	}
