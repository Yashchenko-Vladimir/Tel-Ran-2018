...
public class HWPrintCalendarApp {
	...
	public static void main(String[] args) {
		...
		FIRST_DAY = args[2];
		START_WEEK = getStartWeek(FIRST_DAY);
	...
	private static int getStartWeek(String FirstDay) {
		
		return DayOfWeek.valueOf(FirstDay).getValue() - 1;
	}
...
//		}
	...
	private static int getFirstColumn(DayOfWeek firstDayOfWeek) {
		int res = (int) (firstDayOfWeek.getValue() - START_WEEK);
		if (res < 1) {
		 res = res + 7;
		}
...
//		}
	...
	private static void printTitle(int year, int month) {
		System.out.printf("%8s %d %s", "  ", year, Month.of(month).getDisplayName(TextStyle.FULL, Locale.ENGLISH) + "\n");
