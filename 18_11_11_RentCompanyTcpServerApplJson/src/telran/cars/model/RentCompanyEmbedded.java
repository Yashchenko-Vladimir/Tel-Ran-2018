...
public class RentCompanyEmbedded extends AbstractRentCompany implements Persistable {
	...
	private void addRecordCarRecords(RentRecord record) {
		...
		List<RentRecord> list = carRecords.computeIfAbsent(carNumber, x -> new ArrayList<RentRecord>());
		list.add(record);
	}
	...
	private void addReturnedRecord(LocalDate returnDate, RentRecord tmp) {
		returnedRecords.putIfAbsent(returnDate, new ArrayList<>());
	...
	private void removeRecordsFromAllMaps(Set<Long> licenseId, Set<String> regNumbers) {
	...
	private void removeFromDrivers(Set<Long> licenseIds, Set<String> regNumbers) {
		licenseIds.forEach(li -> {
			List<RentRecord> records = driverRecords.get(li);
			records.removeIf(r -> regNumbers.contains(r.getCarNumber()));
			if (records.isEmpty()) {
	...
	private double getProfitFromRecords(Stream<RentRecord> records, String modelName) {

		return records.collect(Collectors.summingDouble(r -> getProfitFromRecord(r, modelName)));
	}

	private double getProfitFromRecord(RentRecord record, String modelName) {
